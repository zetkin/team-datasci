...
# This whole thing is there to get the regions
# but the regions are unecessary, since we get the data for all of them
...
response = get_scb_data(
    scb_api_path=["AM", "AM0210", "AM0210G", "ArRegDesoStatus"],
    lang="en",
    # region=regions, # regions is unecessary
    observations=[
        "number of employed and unemployed (labour force)",
        "number of unemployed",
    ],  # Title of info in leaf node
    year=["2022"],
    age=["20-64 years"],
)
...
response = get_scb_data(
    ["UF", "UF0506", "UF0506YDeso", "UtbSUNBefDesoRegso"],
    år=["2022"],
    utbildningsnivå=[
...
response_records = [
    {
        ...
        "year": record["key"][2],
        "Befolkning 25-64 år efter region, utbildningsnivå, år och tabellinnehåll": record[
            "values"
        ][
            0
        ],
